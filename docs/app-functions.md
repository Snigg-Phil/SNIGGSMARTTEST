...
### registering the Snigg Smart Charger in the WLAN
...
Open the menu and click on "Connect to WLAN". Your PC must be connected to your router via WLAN. Enter the WLAN password in the "Password" field. The receiver device ID will be used later. Switch on all Snigg Smart Chargers that you want to connect to the WLAN.  
...
**Note**: The WLAN password is encrypted using a general key. Theoretically, it is conceivable that someone knows this general key and can eavesdrop on your WLAN to determine the WLAN password. If you have security concerns about this, it is better to configure the Snigg Smart Controller manually by logging onto its access point. At a later time, you can optionally enter a "receiver device ID" so that the password is encrypted with this in a tap-proof manner. The disadvantage, however, is that you can then only register one Snigg Smart Controller per run in your WLAN.
...
Using the Snigg Cloud, you can access your Snigg Smart Charger and the Snigg Charging Manager with the app (and also via a web browser) while on the move. Here are the [instructions for setting up cloud access](/en/cfos-charging-manager/documentation/cloud.htm).
