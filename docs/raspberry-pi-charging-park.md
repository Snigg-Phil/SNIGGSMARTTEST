...
* [One Snigg Charging Manager charging point licence per wallbox (Snigg Smart Controller wallboxes can be integrated free of charge)](https://shop.cfos-emobility.de/products/cfos-charging-manager-ladepunkt-lizenz)
...
* One USB RS485 adapter (FTDI, virtual COM port) per Modbus RTU  
  The Snigg Smart Charger also has a Modbus proxy. If you have limited EVSE reception or do not want to connect all EVSEs to the network with WLAN, you can use Modbus RTU at these points instead. To do this, connect the EVSEes without WLAN coverage to a EVSE via Modbus RTU and then set up the proxy. The master box then addresses all EVSEs via the Snigg Smart Controller as if they were connected via WLAN or Modbus TCP/IP.
...
2. Register your wallboxes in your network via WLAN (Modbus TCP/IP). Please use the manufacturer's instructions for this if it is not a Snigg Smart Charger.
3. Add all your wallboxes to the settings. Use the following parameters for the device configuration: (Figure 1: Example for Snigg Smart Charger)
...
7. Use the following parameters:  
   ...
   * Solar surplus
8. Save the settings.
9. For more details on surplus charging, balance surplus charging, phase use and electricity storage, see our surplus [charging documentation](/en/cfos-charging-manager/documentation/surplus-charging.htm)
...
[Norsk](/no/cfos-charging-manager/documentation/raspberry-pi-charging-park.htm)
